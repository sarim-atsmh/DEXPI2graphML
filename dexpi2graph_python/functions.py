...
def _generic_attributes(element):
    attrs = {}
    for attribute in element.findall("GenericAttributes/GenericAttribute"):
        name = attribute.get("Name")
        if name and name not in attrs:
            attrs[name] = attribute.get("Value", "")
    return attrs
...
def _combined_tag(prefix="", sequence="", suffix=""):
    return f"{prefix or ''}{sequence or ''}{suffix or ''}".strip()


def _normalized_component_class(component_class):
    mapping = {
        "ProcessInstrumentationFunction": "PCE Request",
        "PlateHeatExchanger": "Heat exchanger",
        "TubularHeatExchanger": "Heat exchanger",
        "PressureVessel": "Vessel",
        "ProcessColumn": "Column",
        "CentrifugalPump": "Liquid pump",
        "ReciprocatingPump": "Liquid pump",
        "PipeTee": "Pipe tee",
        "GlobeValve": "Globe valve",
        "BallValve": "Ball valve",
        "ButterflyValve": "Butterfly valve",
        "SwingCheckValve": "Swing check valve",
        "SpringLoadedGlobeSafetyValve": "Safety valve",
        "BlindFlange": "Flange",
        "OperatedValve": "Valve (general)",
        "FlowMeasuringElement": "Orifice plate",
        "RestrictionOrifice": "Orifice plate",
        "PipeFitting": "Fittings",
        "PipingNetworkSegment": "Piping",
    }
    if component_class in mapping:
        return mapping[component_class]
    if not component_class:
        return ""
    return re.sub(r"(?<=[a-z0-9])(?=[A-Z])", " ", component_class).strip()


def _resolved_tag_name(element, attributes):
    tag_name = _first_non_empty(
        element.get("TagName"),
        attributes.get("TagNameAssignmentClass"),
        attributes.get("PipingComponentNameAssignmentClass"),
        attributes.get("PipingComponentNumberAssignmentClass"),
        attributes.get("PositionNumberAssignmentClass"),
        attributes.get("ActuatingSystemNumberAssignmentClass"),
        attributes.get("FNO"),
    )
    if tag_name:
        return tag_name

    if element.get("ComponentClass") == "ProcessInstrumentationFunction":
        prefix = "".join(
            [
                attributes.get(
                    "ProcessInstrumentationFunctionCategoryAssignmentClass", ""
                ),
                attributes.get("ProcessInstrumentationFunctionsAssignmentClass", ""),
            ]
        ).strip()
        number = attributes.get(
            "ProcessInstrumentationFunctionNumberAssignmentClass", ""
        ).strip()
        if prefix or number:
            return " ".join([part for part in [prefix, number] if part])

    combined_tag = _combined_tag(
        attributes.get("TagNamePrefixAssignmentClass", ""),
        attributes.get("TagNameSequenceNumberAssignmentClass", ""),
        attributes.get("TagNameSuffixAssignmentClass", ""),
    )
    if combined_tag:
        return combined_tag

    component_class = _normalized_component_class(element.get("ComponentClass"))
    identifier = element.get("ID", "")
    if component_class and identifier:
        return f"{component_class}_{identifier.split('-')[-1]}"
    return identifier or component_class or "Unnamed"


def _resolved_position(element, fallback_index, fallback_y):
    location = element.find("Position/Location")
    if location is None:
        location = element.find(".//Position/Location")
    if (
        location is not None
        and location.get("X") not in (None, "")
        and location.get("Y") not in (None, "")
    ):
        return (
            float(location.get("X").replace(",", ".")),
            float(location.get("Y").replace(",", ".")),
        )
    return (float(fallback_index * 100), float(fallback_y))


def _association_targets(element, association_type):
    targets = []
    for association in element.findall("Association"):
        if association.get("Type") == association_type and association.get("ItemID"):
            targets.append(association.get("ItemID"))
    return targets


def _resolved_sub_class(element, attributes):
    component_class = element.get("ComponentClass")
    piping_sub_class = {
        "PrimaryPipingNetworkSegment": "Main pipe",
        "SecondaryPipingNetworkSegment": "Secondary pipe",
    }
    return _first_non_empty(
        attributes.get("SUB_CLASS"),
        piping_sub_class.get(
            attributes.get("PrimarySecondaryPipingNetworkSegmentSpecialization"), ""
        ),
        attributes.get("LocationSpecialization"),
        attributes.get("NominalDiameterRepresentationAssignmentClass"),
        "General",
    )


def _build_aliases(root):
    aliases = {}
    nozzle_to_equipment = {}

    for equipment in root.findall("Equipment"):
        equipment_id = equipment.get("ID")
        for nozzle in equipment.findall(".//Nozzle"):
            nozzle_id = nozzle.get("ID")
            if nozzle_id and equipment_id:
                nozzle_to_equipment[nozzle_id] = equipment_id
                aliases[nozzle_id] = equipment_id

    for signal_generator in root.findall(".//ProcessSignalGeneratingFunction"):
        signal_generator_id = signal_generator.get("ID")
        for location_id in _association_targets(signal_generator, "is located in"):
            resolved_location = nozzle_to_equipment.get(location_id, location_id)
            if signal_generator_id and resolved_location:
                aliases[signal_generator_id] = resolved_location
                break

    for actuating_system in root.findall("ActuatingSystem"):
        target_valve = ""
        for system_component in actuating_system.findall("ActuatingSystemComponent"):
            if system_component.get("ComponentClass") == "OperatedValveReference":
                target_valve = _first_non_empty(
                    *_association_targets(system_component, "refers to")
                )
                if target_valve:
                    break
        if not target_valve:
            continue
        for actuating_function_id in _association_targets(actuating_system, "fulfills"):
            aliases[actuating_function_id] = target_valve

    return aliases


def _resolve_graph_id(identifier, aliases):
    resolved = identifier
    visited = set()
...
def Dexpi2graph(
    ...
    for equipment in myroot.findall("Equipment"):  # all equipments
        ...
        # read
        ID = equipment.get("ID")
        equipment_attributes_map = _generic_attributes(equipment)
        N = _resolved_tag_name(equipment, equipment_attributes_map)

        # read equipment coordinates.
        x, y = _resolved_position(equipment, i, 0)

        # Read all equipment attributes
        for equipment_attributes in equipment.findall(
            ...
            if equipment_attributes.get("Name") == "CLASS":  # selecting the class
                ...
                equipment_attributes.get("Name") == "SUB_CLASS"
            ):  # selecting the sub class
                C_sub = equipment_attributes.get("Value")

            elif (
                equipment_attributes.get("Name") == "VPE_PRESSURE_DESIGN_MAX"
            ):  # selecting max design pressure
                # In general, entry is a string containing value and unit, so the string has to be tokenized
                if (
                    len(nltk.word_tokenize(equipment_attributes.get("Value"))) == 2
                ):  # Assure correct entry
                    P1 = float(
                        nltk.word_tokenize(equipment_attributes.get("Value"))[
                            0
                        ].replace(",", ".")
                    )
                ...
                equipment_attributes.get("Name") == "VPE_PRESSURE_DESIGN_MIN"
            ):  # selecting min design pressure
                if len(nltk.word_tokenize(equipment_attributes.get("Value"))) == 2:
                    P2 = float(
                        nltk.word_tokenize(equipment_attributes.get("Value"))[
                            0
                        ].replace(",", ".")
                    )  # first token of dexpi string is selected
                ...
                equipment_attributes.get("Name") == "VPE_TEMP_DESIGN_MAX"
            ):  # selecting max design temperature
                if len(nltk.word_tokenize(equipment_attributes.get("Value"))) == 2:
                    T1 = float(
                        nltk.word_tokenize(equipment_attributes.get("Value"))[
                            0
                ...
                equipment_attributes.get("Name") == "VPE_TEMP_DESIGN_MIN"
            ):  # selecting min design temperature
                if len(nltk.word_tokenize(equipment_attributes.get("Value"))) == 2:
                    T2 = float(
                        nltk.word_tokenize(equipment_attributes.get("Value"))[
                            0
                ...
                equipment_attributes.get("Name") == "VPE_MAT_PARTS_MEDIA_CONTACT"
            ):  # selecting material in contact with medium
                M = equipment_attributes.get("Value")

            elif (
                equipment_attributes.get("Name") == "VPE_TNK_VOL_BRUTTO"
            ):  # selecting volume
                if len(nltk.word_tokenize(equipment_attributes.get("Value"))) == 2:
                ...
                equipment_attributes.get("Name") == "INSULATION"
            ):  # selecting insulation (Yes/No)
                I = equipment_attributes.get("Value")

            elif (
                equipment_attributes.get("Name") == "COOLING_HEATING_SYSTEM"
            ):  # selecting cooling/heating system
                CH = equipment_attributes.get("Value")

            elif (
                equipment_attributes.get("Name") == "FN_LOCATION"
            ):  # selecting location
                L = equipment_attributes.get("Value")  # outside

        if C == "Not available":
            C = _normalized_component_class(equipment.get("ComponentClass"))
        if C_sub == "Not available":
            C_sub = _resolved_sub_class(equipment, equipment_attributes_map)
        ...
        graph.add_node(
            ...
            node_name=N,
            node_class=C,
            node_sub_class=C_sub,
            node_material=M,
            node_unit_V=unit_V,
            node_P_max=P1,
            node_unit_P1=unit_P1,
            node_P_min=P2,
            node_unit_P2=unit_P2,
            node_T_max=T1,
            node_unit_T1=unit_T1,
            node_T_min=T2,
            node_unit_T2=unit_T2,
            node_volume=V,
            node_insulation=I,
            node_cool_heat_system=CH,
            node_x=x,
            node_y=y,
            node_location=L,
            node_request=R,
    ...
    for PIF in myroot.findall("ProcessInstrumentationFunction"):  # all MSR
        ...
        # Read
        ID = PIF.get("ID")
        pif_attributes_map = _generic_attributes(PIF)
        N = _resolved_tag_name(PIF, pif_attributes_map)

        # Read equipment coordinates
        x, y = _resolved_position(PIF, i, 400)
        ...
        for PIF_attributes in PIF.findall("GenericAttributes/GenericAttribute"):
            ...
            if (
                PIF_attributes.get("Name") == "PCE_CAT_FUNC"
        ...
        if C == "Not available":
            C = _normalized_component_class(PIF.get("ComponentClass"))
        if C_sub == "Not available":
            C_sub = _resolved_sub_class(PIF, pif_attributes_map)
        if R == "Not available":
            R = "".join(
                [
                    pif_attributes_map.get(
                        "ProcessInstrumentationFunctionCategoryAssignmentClass", ""
                    ),
                    pif_attributes_map.get(
        ...
        if L == "Not available":
            L = pif_attributes_map.get("LocationSpecialization", "Not available")
    ...
    for piping_component in myroot.findall(
        "PipingNetworkSystem//PipingComponent"
    ):  # select all piping components
        ...
        piping_attributes_map = _generic_attributes(piping_component)
        N = _resolved_tag_name(piping_component, piping_attributes_map)

        # Read equipment coordinates
        x, y = _resolved_position(piping_component, i, 200)

        if (
            piping_component.get("ComponentClass") in (None, "")
            and "CLASS" not in piping_attributes_map
            and "PipingComponentNumberAssignmentClass" not in piping_attributes_map
            and "PositionNumberAssignmentClass" not in piping_attributes_map
        ):
            continue

        # in case of a pipe tee
        normalized_component_class = _normalized_component_class(
            piping_component.get("ComponentClass")
        )
        if normalized_component_class == "Pipe tee":

            C = normalized_component_class
            C_sub = _resolved_sub_class(piping_component, piping_attributes_map)
            N = "Pipe_tee_" + str(
                j
            )  # P&ID name does not exist, so name by consecutive numbers
        ...
        # in case of an in/-outlet
        elif normalized_component_class in [
            "Arrow",
            "Flow in pipe connector symbol",
            "Flow out pipe connector symbol",
        ] or piping_component.get("ComponentClass") in [
            "Arrow",
            "Flow in pipe connector symbol",
            "Flow out pipe connector symbol",
        ]:
            ...
            C = _first_non_empty(
                normalized_component_class, piping_component.get("ComponentClass")
            )
            C_sub = _resolved_sub_class(piping_component, piping_attributes_map)
            N = "C_" + str(
                k
            ...
            # Read attributes
            for Attribute in piping_component.findall(
                "GenericAttributes/GenericAttribute"
            ):
        ...
        # other piping components
            ...
            # Read attributes
            for piping_component_attribute in piping_component.findall(
                "GenericAttributes/GenericAttribute"
            ):
    ...
    for PNS in myroot.findall("PipingNetworkSystem/PipingNetworkSegment"):
        ...
        pns_attributes_map = _generic_attributes(PNS)

        # identificate connection
        for connection in PNS.findall("Connection"):
            FromID = _resolve_graph_id(connection.get("FromID"), graph_aliases)
            ToID = _resolve_graph_id(connection.get("ToID"), graph_aliases)
            ...
            if FromID in list(graph.nodes()) and ToID in list(graph.nodes()):
                ...
                if (
                    ...
                    Nu = "Not available"
                    C = _first_non_empty(
                        pns_attributes_map.get("CLASS"),
                        _normalized_component_class(PNS.get("ComponentClass")),
                    )
                    C_sub = _resolved_sub_class(PNS, pns_attributes_map)

                    # Read attributes
                    for piping_attribute in PNS.findall(
                        "GenericAttributes/GenericAttribute"
                    ):

                        if (
                            piping_attribute.get("Name") == "CLASS"
                        ):  # selecting the class
                            C = piping_attribute.get("Value")

                        elif (
                            piping_attribute.get("Name") == "SUB_CLASS"
                        ):  # selecting the sub class
                            C_sub = piping_attribute.get("Value")

                        elif (
                            piping_attribute.get("Name") == "VPE_MAT_MAIN_MATERIAL"
                        ):  # selecting the material
                            M = piping_attribute.get("Value")

                        elif (
                            piping_attribute.get("Name") == "NOMINAL_DIAMETER"
                        ):  # selecting Diameter
                            D = piping_attribute.get("Value")

                        elif (
                            piping_attribute.get("Name") == "MAT_INAME"
                        ):  # selecting pipe class
                            C_pipe = piping_attribute.get("Value")

                        elif (
                            piping_attribute.get("Name") == "PIPENO"
                        ):  # selecting Pipe number
                            Nu = piping_attribute.get("Value")

                    if M == "Not available":
                        M = _first_non_empty(
                            pns_attributes_map.get("VPE_MAT_MAIN_MATERIAL"),
                            pns_attributes_map.get("FluidCodeAssignmentClass"),
                            M,
                        )
                    if D == "Not available":
                        D = _first_non_empty(
                            pns_attributes_map.get("NOMINAL_DIAMETER"),
                            pns_attributes_map.get(
                    ...
                    if C_pipe == "Not available":
                        C_pipe = _first_non_empty(
                            pns_attributes_map.get("MAT_INAME"),
                            pns_attributes_map.get("PipingClassCodeAssignmentClass"),
                            C_pipe,
                        )
                    if Nu == "Not available":
                        Nu = _first_non_empty(
                            pns_attributes_map.get("PIPENO"),
                            pns_attributes_map.get("SegmentNumberAssignmentClass"),
                            Nu,
                        )
                    ...
                    graph.add_edge(
                        ...
                        ToID,
                        edge_class=C,
                        edge_sub_class=C_sub,
                        edge_material=M,
                        edge_diameter=D,
                        edge_pipe_class=C_pipe,
                        edge_number=Nu,
                    )  # Adding connection as edge to graph
            ...
            # Notice node(s) that was not registrated before
                ...
                if (
                    FromID not in list(graph.nodes())
                    and FromID != None
                    and FromID != ""
                ):
    ...
    if nodes_from_nothing != []:
        Error_log.loc[k, "Warning"] = (
            "There is at least one node without a source. Please make sure it is correct."
        )
        Error_log.loc[k, "Node(s)"] = str(nodes_from_nothing)
        k += 1
    if nodes_to_nothing != []:
        Error_log.loc[k, "Warning"] = (
            "There is at least one node without a destination. Please make sure it is correct."
        )
        Error_log.loc[k, "Node(s)"] = str(nodes_to_nothing)
        k += 1
    if nodes_not_registrated != []:
        Error_log.loc[k, "Warning"] = (
            "At least one exported edge contains a node that was not registrated before."
        )
    ...
    for InfoFlow in myroot.findall("ProcessInstrumentationFunction/InformationFlow"):
        ...
        infoflow_attributes_map = _generic_attributes(InfoFlow)
        connections = [
            (connection.get("FromID"), connection.get("ToID"))
            for connection in InfoFlow.findall("Connection")
        ]
        if connections == []:
            starts = _association_targets(InfoFlow, "has logical start")
            ends = _association_targets(InfoFlow, "has logical end")
            connections = [(start, end) for start in starts for end in ends]

        for raw_from_id, raw_to_id in connections:
            FromID = _resolve_graph_id(raw_from_id, graph_aliases)
            ToID = _resolve_graph_id(raw_to_id, graph_aliases)

            # Connection only added if there is a start and end point
            if (
                FromID in list(graph.nodes())
                and ToID in list(graph.nodes())
                and FromID not in (None, "")
                and ToID not in (None, "")
                and FromID != ToID
            ):
                ...
                Nu = "Not available"
                C = _first_non_empty(
                    infoflow_attributes_map.get("CLASS"),
                    {
                        "MeasuringLineFunction": "Process connection line",
                        "SignalLineFunction": "Signal line",
                        "SignalConveyingFunction": "Signal line",
                    }.get(InfoFlow.get("ComponentClass"), ""),
                    "Signal line",
                )
                C_sub = _first_non_empty(
                    infoflow_attributes_map.get("SUB_CLASS"),
                    _normalized_component_class(InfoFlow.get("ComponentClass")),
                    "General",
                )

                # Read attributes
                for attributes in InfoFlow.findall(
                    "GenericAttributes/GenericAttribute"
                ):
    ...
    for node in graph.nodes():
        if nx.is_isolate(graph, node) and node_class[node] not in [
            "Agitator",
            "Orifice plate",
        ]:
    ...
    if nodes_isolated != {} and len(nodes_isolated) < len(graph.nodes()):
        Error_log.loc[k, "Warning"] = (
            "Isolated nodes were identificated and removed. Please make sure it is correct."
        )
        Error_log.loc[k, "Node(s)"] = str(nodes_isolated)
        k += 1

    if len(nodes_isolated) < len(graph.nodes()):
    ...
    pipe_attributes = {
        ...
        "Piping, heating or cooled insulated": {
            "Insulation": "Yes",
            "Heated/cooled": "Yes",
        },
    }  # preparation for creating new edge with new attributes (dict to avoid repeating script)

    # Add following new attributes to the already existing nodes
    for edge in graph.edges():
        if edge_class[edge] == "Piping":
            graph.add_edge(
                edge[0], edge[1], edge_insulation="No", edge_heated_cooled="No"
            )
        else:
            graph.add_edge(
                edge[0],
                edge[1],
                edge_insulation="Not available",
                edge_heated_cooled="Not available",
            )
    ...
    for node in graph.nodes():
        if node_class[node] in ["Hose", "Pipe equipment"]:
            ...
            if (
                len(list((graph.in_edges(node)))) == 1
                and len(list((graph.out_edges(node)))) == 1
            ):
                remove.append(node)
                    ...
                    for sub_class in pipe_attributes:
                        if node_sub_class[node] == sub_class:
                            graph.add_edge(
                                ...
                                edge_pipe_class=C_pipe,
                                edge_number=Nu,
                                edge_insulation=pipe_attributes[sub_class][
                                    "Insulation"
                                ],
                                edge_heated_cooled=pipe_attributes[sub_class][
                                    "Heated/cooled"
                                ],
                            )
    ...
    if nodes_problem != []:
        Error_log.loc[k, "Warning"] = (
            "Problems in converting hose and piping components. At least one node is connected in an unexpected way."
        )
    ...
    graph.remove_nodes_from(
        remove
    )  # remove old nodes for pipe equipment and nodes from graph
    ...
    dict_group = {
        ...
        "Valves/Fittings": [
            ...
            "Safety valve",
            "Safety angle valve",
            "Safety valve, angled type",
            "Swing check valve",
            "Three-way ball valve",
            "Three-way globe valve",
            "Three-way valve, general",
            "Valve (general)",
            "Valve, angle ball type",
            "Valve, angle globe type",
            "Valve, angle type (general)",
            "Valve, ball type",
        ...
        "Heat exchanger": [
            "Heat exchanger",
            "Heat exchanger ",
            "Heat exchanger, detailed",
            "Spiral type heat exchanger",
            "Heat exchanger",
            "detailed, Tube bundle with U-tubes",
            "Electric Heaters",
            "Facility for heating or cooling",
        ],
        "Pipe tee": ["Pipe tee"],
        "Connector": [
            "Arrow",
    ...
    dict_sub_group = {
        "Vessel": {
            "Vessel (solid)": {
                "Silo": "General",
                "Bag": "General",
                "Container": "Container for solids",
            },
        ...
        "Valves/Fittings": {
            "Valve (safety)": {
                "Safety valve": ["spring loaded", "General"],
                "Safety angle valve": ["Spring loaded", "General"],
                "Safety valve, angled type": ["General", "spring loaded"],
                "Flame arrestor": "all",
                "Rupture disk": "all",
            },
        ...
        "Filter": {
            "Filter (gaseous)": {"Air filter": "General", "Gas filter": "all"},
            "Filter (liquid)": "All other",
        },
        "Heat exchanger": {
            "Heat exchanger (electric)": {
                "Electric Heaters": "General",
                "Facility for heating or cooling": "General",
            },
            "Heat exchanger (medium)": "All other",
        },
    }  # sub groups
        ...
        for group in dict_sub_group:  # regarding all groups
            if node_group[node] == group:  # select nodes belongs to the regarded group
                for sub_group in dict_sub_group[group]:  # regarding all sub groups
                    ...
                    if (
                        dict_sub_group[group][sub_group] == "All other"
                    ):  # everything else
                        ...
                        for Class in dict_sub_group[group][sub_group]:
                            if Class == node_class[node]:
                                ...
                                if type(sub_classes) == str:
                                    if (
                                        sub_classes == node_sub_class[node]
                                        or sub_classes == "all"
                                    ):
                                        graph.add_node(node, node_sub_group=sub_group)
                                        grouped = "Yes"
        ...
        if node_class[node] == "Agitator":
            ...
            for other_node in graph.nodes():
                if node_group[other_node] == "Vessel":
                    ...
                    Position_difference = abs(x_agitator - x_other_node) + abs(
                        y_agitator - y_other_node
                    )  # Calculate Position difference
                    Position[Position_difference] = (
                        other_node  # save distance together with node
                    )
        ...
        if node_class[node] == "Orifice plate":
            ...
            for other_node in graph.nodes():
                if node_class[other_node] == "Flange":
                    ...
                    Position_difference = abs(x_flange - x_other_node) + abs(
                        y_fange - y_other_node
                    )
                    Position[Position_difference] = (
                        other_node  # save distance togehter with node
                    )
            ...
            if Position != {}:
                ...
                graph.add_node(
                    ...
                    node_sub_class=node_sub_class[node],
                    node_x=node_x[node],
                    node_y=node_y[node],
                    node_group=node_group[node],
                    node_sub_group=node_sub_group[node],
                )
    ...
    if no_label_nodes != []:
        Error_log.loc[k, "Warning"] = (
            "Warning: At least one node is not labeled. Label of such a node was exchanged with ID of DEXPI-Export. To avoid it, please add a label for the node in the P&ID."
        )
    ...
    for node_1 in graph.nodes():
        ...
        for node_2 in graph.nodes():
            j += 1
            if (
                node_name[node_1] == node_name[node_2]
                and i != j
                and node_name[node_1] not in double_names
            ):  # avoid similarity because of same node and noticing a name for more than one time in the list
    ...
    if double_names != []:
        Error_log.loc[k, "Warning"] = (
            "At least one label is used more than one time. That is not possible cause a clear assignment must be given. For this run the nodes are numbered. For next run, please use clear label."
        )
    ...
    if nodes_wrong_connection != []:
        Error_log.loc[k, "Warning"] = (
            "At least one node is wrong connected in the P&ID. Please check P&ID for the next run"
        )
    ...
    for node in graph_no_MSR.nodes():
        if "PIF" in node_ID_MSR[node]:
            ...
            if len(all_neighbors) == 1:
                ...
                if (
                    len(all_neighbors) == 2
                    and node_group_MSR[node_neighbor] == "Valves/Fittings"
                ):
    ...
    nx.write_graphml(
        graph_no_MSR,
        save_path_graph_no_MSR + savename + "_noMSR.xml",
        encoding="utf-8",
        prettyprint=True,
        infer_numeric_types=False,
    )
...
# Dexpi2graph(
#     "/home/rapidswords/SMH Coders/Kent/Generating-DEXPI/dexpi 1.3/example pids/C01 DEXPI Reference P&ID/C01V04-VER.EX01.xml",
#     "./Output/graphs_graphml/complete/",
#     "./Output/graphs_graphml/noPCE/",
#     "./Output/NodeLists/",
#     "./Output/errorLog/",
#     "output_custom",
# )
